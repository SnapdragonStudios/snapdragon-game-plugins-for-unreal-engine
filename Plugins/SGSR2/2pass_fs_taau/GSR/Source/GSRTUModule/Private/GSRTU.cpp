...
class FGSRConvertPS : public FGSRShader
{
	DECLARE_GLOBAL_SHADER(FGSRConvertPS);
	SHADER_USE_PARAMETER_STRUCT(FGSRConvertPS, FGSRShader);
...
class FGSRUpscalePS : public FGSRShader
{
	DECLARE_GLOBAL_SHADER(FGSRUpscalePS);
	SHADER_USE_PARAMETER_STRUCT(FGSRUpscalePS, FGSRShader);
...
IMPLEMENT_GLOBAL_SHADER(FGSRConvertPS, "/Plugin/GSR/Private/gsr_convert.usf", "MainPS", SF_Pixel);
IMPLEMENT_GLOBAL_SHADER(FGSRUpscalePS, "/Plugin/GSR/Private/gsr_upscale.usf", "MainPS", SF_Pixel);
	...
	FIntRect* OutSceneColorHalfResViewRect) const
{
	...
	if ((View.PrimaryScreenPercentageMethod == EPrimaryScreenPercentageMethod::TemporalUpscale) && bHasAutoExposure)
	{
		...
		/////////
		{
			FGSRConvertPS::FParameters* PassParameters = GraphBuilder.AllocParameters<FGSRConvertPS::FParameters>();
			...
			FGSRConvertPS::FPermutationDomain PermutationVector;

			TShaderMapRef<FGSRConvertPS> PixelShader(View.ShaderMap, PermutationVector);

			const FScreenPassTextureViewport InputViewport(InputRect);
			...
			GEngine->AddOnScreenDebugMessage(0, 1.0f, FColor::Red, FString::Printf(TEXT("SGSR2 Convert %dx%d -> %dx%d (PS)"), InputViewport.Rect.Width(), InputViewport.Rect.Height(), InputViewport.Rect.Width(), InputViewport.Rect.Height()));
		}
		...
		{
			FGSRUpscalePS::FParameters* PassParameters = GraphBuilder.AllocParameters<FGSRUpscalePS::FParameters>();
			PassParameters->CommonParameters = CommonParameters;
			...
			FGSRUpscalePS::FPermutationDomain PermutationVector;
			TShaderMapRef<FGSRUpscalePS> PixelShader(View.ShaderMap, PermutationVector);
			...
			AddDrawScreenPass(GraphBuilder, RDG_EVENT_NAME("Qualcomm-SGSR2/Upscale (PS)"), View, OutputViewport, InputViewport, PixelShader, PassParameters);
			GEngine->AddOnScreenDebugMessage(1, 1.0f, FColor::Yellow, FString::Printf(TEXT("SGSR2 Upscale %dx%d -> %dx%d (PS)"), InputViewport.Rect.Width(), InputViewport.Rect.Height(), OutputViewport.Rect.Width(), OutputViewport.Rect.Height()));
		}

		if (bWritePrevViewInfo)
		{
			GraphBuilder.QueueTextureExtraction(ColorOutputTexture, &OutputHistory->RT[0]);
			GraphBuilder.QueueTextureExtraction(Output2.Texture, &OutputHistory->RT[1]);
		}
