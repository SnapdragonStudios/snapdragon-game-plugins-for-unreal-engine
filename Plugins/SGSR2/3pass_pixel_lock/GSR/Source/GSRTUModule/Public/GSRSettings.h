...
public:
	...
	UPROPERTY(Config, EditAnywhere, Category = "Quality Settings", meta = (ConsoleVariable = "r.SGSR2.PixelLock", DisplayName = "Pixel Lock", ToolTip = "Enable pixel lock for better thin feature upscaling (small performance hit)."))
	bool PixelLock;

	//UPROPERTY(Config, EditAnywhere, Category = "Quality Settings", meta = (ConsoleVariable = "r.SGSR2.PixelLockReproject", DisplayName = "Pixel Lock Reproject", ToolTip = "Enable pixel lock reprojection for (even) better thin feature upscaling (small performance hit, needs Pixel Lock to be enabled)."))
	//bool PixelLockReproject;

	UPROPERTY(Config, EditAnywhere, Category = "Quality Settings", meta = (ConsoleVariable = "r.SGSR2.HalfPrecision", DisplayName = "Half Precision", ToolTip = "Use half precision (16bit) arithmetic when platform support is available."))
	bool Halfprecision;
