...
class FGSRConvertCS : public FGSRShader
{
	...
	BEGIN_SHADER_PARAMETER_STRUCT(FParameters, )
		...
		SHADER_PARAMETER_RDG_TEXTURE_SRV(Texture2D, InputVelocitySRV)
...
public:
	...
	void SetSceneTexturesUniformBuffer(FRHIUniformBuffer* InSceneTexturesUniformParams)
	...
	: ReflectionTexture(nullptr)
{
	...
	{
		SavedSP = IConsoleManager::Get().FindTConsoleVariableDataFloat(TEXT("r.ScreenPercentage"))->GetValueOnGameThread();
...
ITemporalUpscaler::FOutputs FGSRTU::AddPasses(
	FRDGBuilder& GraphBuilder,
	const FViewInfo& View,
	const ITemporalUpscaler::FPassInputs& PassInputs) const
{
	///output extent
	...
	if ((View.PrimaryScreenPercentageMethod == EPrimaryScreenPercentageMethod::TemporalUpscale) && bHasAutoExposure)
	{
		...
		RDG_EVENT_SCOPE(GraphBuilder, "SGSR3Pass");
		...
		/////////
		{
			...
			PassParameters->InputVelocitySRV = GraphBuilder.CreateSRV(FRDGTextureSRVDesc::Create(PassInputs.SceneVelocityTexture));
...
ITemporalUpscaler* FGSRTU::Fork_GameThread(const class FSceneViewFamily& InViewFamily) const
{
...
void FGSRTU::CopyOpaqueColor(FRDGBuilder& GraphBuilder, TConstArrayView<FViewInfo> Views, const class FShaderParametersMetadata* SceneTexturesUniformBufferStruct, FRHIUniformBuffer* SceneTexturesUniformBuffer)
{
...
FGSRTUFork::FGSRTUFork(ITemporalUpscaler* TU)
	: TemporalUpscaler(TU)
{
}
...
ITemporalUpscaler::FOutputs FGSRTUFork::AddPasses(
	FRDGBuilder& GraphBuilder,
	const FViewInfo& View,
	const ITemporalUpscaler::FPassInputs& PassInputs) const
{
...
ITemporalUpscaler* FGSRTUFork::Fork_GameThread(const class FSceneViewFamily& InViewFamily) const
{
