...
#include "../Shaders/Private/GSR1/sgsr.ush"
...
IMPLEMENT_GLOBAL_SHADER(FSGSRPS, "/Plugin/SGSR/Private/GSR1/PostProcessSGSR.usf", "MainPS", SF_Pixel);
...
void FSGSRSubpassScaler::CreateResources(FRDGBuilder& GraphBuilder, const FViewInfo& View, const FInputs& PassInputs)
{
	...
	Data->bSGSREnabled = CVarEnabled && CVarEnabled->GetValueOnAnyThread() && (Data->InputViewport.Rect != Data->OutputViewport.Rect);
...
void FSGSRSubpassScaler::Upscale(FRDGBuilder& GraphBuilder, const FViewInfo& View, const FInputs& PassInputs)
{
	...
	{
		GEngine->AddOnScreenDebugMessage(0, 1.0f, FColor::Red, FString::Printf(TEXT("SGSR Spatial Upscaler")));
		...
		static const auto CVarHalfPrecisionEnabled = IConsoleManager::Get().FindTConsoleVariableDataBool(TEXT("r.SGSR.HalfPrecision"));
		bool IsHalfPrecisionEnabled = CVarHalfPrecisionEnabled ? CVarHalfPrecisionEnabled->GetValueOnAnyThread() : false;
		PermutationVector.Set<FSGSRPS::FSGSR_HalfPrecision>(IsHalfPrecisionEnabled);
		...
		GEngine->AddOnScreenDebugMessage(1, 1.0f, FColor::Orange, FString::Printf(TEXT("SGSR - Upscale %dx%d -> %dx%d (PS)"), Data->InputViewport.Rect.Width(), Data->InputViewport.Rect.Height(), Data->OutputViewport.Rect.Width(), Data->OutputViewport.Rect.Height()));
	}
}