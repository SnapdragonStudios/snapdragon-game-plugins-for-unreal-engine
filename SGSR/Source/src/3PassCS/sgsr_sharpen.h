...
void CurrFilter(uint2 pos)
{
	...
	half3 b = Input[sp + int2(0, -1)].rgb;
	half3 f = Input[sp + int2(1, 0)].rgb;
	half3 min4YCoCg = min(b, f);
	half3 max4YCoCg = max(b, f);
	half3 h = Input[sp + int2(0, 1)].rgb;
	min4YCoCg = min(min4YCoCg, h);
	max4YCoCg = max(max4YCoCg, h);
	half3 d = Input[sp + int2(-1, 0)].rgb;
	...
	half3 e = Input[sp].rgb;
		//PrepareRgb(Input[sp].rgb, PreExposure);
	...
	UpscaledOutput[pos] = half4(pixY, pixCo, pixCg, 1.0);
}
