...
void Convert(uint2 DisThreadID)
{
	...
	half NearestZ = FindNearestDepth(DisThreadID, uint2(DepthInfo_ViewportSize), fNearestDepthCoord);
	...
	half3 ycocg = RGBToYCoCg(ColorPostAlpha);
	YCoCgLuma[DisThreadID] = half4(ycocg, 0.0);
	half3 fPreparedRgb = max(half3(0, 0, 0), ColorPostAlpha);
